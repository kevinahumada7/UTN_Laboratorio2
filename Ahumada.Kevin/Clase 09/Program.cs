...
namespace Clase_09
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Console.WriteLine("Titulo: {0}", miLibro.getTitulo);

            Console.WriteLine("Autor: {0}", miLibro.getAutor);

            Console.WriteLine("Cantidad de páginas: {0}", miLibro.CantidadDePaginas);

            for (int i = 0; i < miLibro.CantidadDeCapitulos; i++)
            {
                Console.WriteLine("Capitulo {0}: {1} {2}", miLibro[i].getNumero, miLibro[i].getTitulo, miLibro[i].getPaginas);
            }
